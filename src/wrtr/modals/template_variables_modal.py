"""
Modal to collect values for template variables.

Given a list of variable names, it renders Input fields and returns a dict
mapping variable -> value when accepted, or None on dismiss.
"""
...
class TemplateVariablesModal(PaletteDismissModal[Dict[str, str] | None]):
    ...
    def compose(self) -> Iterable:
        with Vertical(id="dialog", classes="dialog-box"):
            yield Label("Fill template variables:")
            # Create an Input per variable
            for v in self.variables:
                yield Label(v)
                yield Input(placeholder=v, id=f"var-{v}")
    ...
    def on_button_pressed(self, event) -> None:
        if event.button.id == "apply":
            result = {}
            for v in self.variables:
                inp = self.query_one(f"#var-{v}", Input)
                result[v] = inp.value
            self.dismiss(result)
        else:
            self.dismiss(None)
